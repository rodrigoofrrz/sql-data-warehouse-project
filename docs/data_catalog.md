# Catálogo de Dados da Camada Ouro

## Visão Geral

A Camada Ouro é a representação de dados em nível de negócio, estruturada para dar suporte a casos de uso analíticos e de relatórios. Ela é composta por **tabelas de dimensão** e **tabelas fato** para métricas de negócio específicas.

---

### 1. **gold.dim_customers**

- **Finalidade:** Armazena detalhes dos clientes enriquecidos com dados demográficos e geográficos.
- **Colunas:**
...
| birthdate       | DATE         | A data de nascimento do cliente, formatada como YYYY-MM-DD (ex.: 1971-10-06).                                  |
...
| Nome da Coluna       | Tipo de Dado | Descrição                                                                                                                  |
| -------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------- |
| product_key          | INT          | Chave substituta (surrogate key) que identifica de forma única cada registro de produto na tabela de dimensão de produtos. |
| product_id           | INT          | Um identificador único atribuído ao produto para rastreamento e referência internos.                                       |
| product_number       | NVARCHAR(50) | Um código alfanumérico estruturado que representa o produto, geralmente usado para categorização ou controle de estoque.   |
| product_name         | NVARCHAR(50) | Nome descritivo do produto, incluindo detalhes importantes como tipo, cor e tamanho.                                       |
| category_id          | NVARCHAR(50) | Um identificador único para a categoria do produto, ligando-o à sua classificação de alto nível.                           |
...
| start_date           | DATE         | A data em que o produto ficou disponível para venda ou uso.                                                                |
...
| Nome da Coluna | Tipo de Dado | Descrição                                                                                         |
| -------------- | ------------ | ------------------------------------------------------------------------------------------------- |
| order_number   | NVARCHAR(50) | Um identificador alfanumérico único para cada pedido de venda (ex.: 'SO54496').                   |
| product_key    | INT          | Chave substituta (surrogate key) que liga o pedido à tabela de dimensão de produtos.              |
| customer_key   | INT          | Chave substituta (surrogate key) que liga o pedido à tabela de dimensão de clientes.              |
| order_date     | DATE         | A data em que o pedido foi realizado.                                                             |
| shipping_date  | DATE         | A data em que o pedido foi enviado ao cliente.                                                    |
| due_date       | DATE         | A data em que o pagamento do pedido venceu.                                                       |
| sales_amount   | INT          | O valor monetário total da venda para o item da linha, em unidades monetárias inteiras (ex.: 25). |
| quantity       | INT          | A quantidade de unidades do produto pedidas para o item da linha (ex.: 1).                        |
